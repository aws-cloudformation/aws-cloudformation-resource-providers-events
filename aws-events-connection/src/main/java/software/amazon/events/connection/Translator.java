...
public class Translator {
    ...
    private static CreateConnectionAuthRequestParameters translateToSdkCreateAuthRequestParameter(final AuthParameters authParameters) {
        ...
        HttpParameters modelInvocationHttpParameters = authParameters.getInvocationHttpParameters();
    ...
    private static ConnectionHttpParameters translateToSdkConnectionHttpParameters(HttpParameters httpParameters) {
        if (httpParameters != null) {
            List<ConnectionHeaderParameter> headerParameters = ListConverter.toSdk(httpParameters.getHeaderParameters(), x -> ConnectionHeaderParameter.builder().key(x.getKey()).value(x.getValue()).isValueSecret(x.getIsValueSecret()).build());
            List<ConnectionBodyParameter> bodyParameters = ListConverter.toSdk(httpParameters.getBodyParameters(), x -> ConnectionBodyParameter.builder().key(x.getKey()).value(x.getValue()).isValueSecret(x.getIsValueSecret()).build());
            List<ConnectionQueryStringParameter> queryStringParameters = ListConverter.toSdk(httpParameters.getQueryStringParameters(), x -> ConnectionQueryStringParameter.builder().key(x.getKey()).value(x.getValue()).isValueSecret(x.getIsValueSecret()).build());
    ...
    private static HttpParameters translateFromSdkToModelHttpParameters(final ConnectionHttpParameters params) {
        if (params != null) {
            List<Parameters> headerParameters = ListConverter.toModel(params.headerParameters(), x -> Parameters.builder().key(x.key()).value(x.value()).isValueSecret(x.isValueSecret()).build());
            List<Parameters> bodyParameters = ListConverter.toModel(params.bodyParameters(), x -> Parameters.builder().key(x.key()).value(x.value()).isValueSecret(x.isValueSecret()).build());
            List<Parameters> queryStringParameters = ListConverter.toModel(params.queryStringParameters(), x -> Parameters.builder().key(x.key()).value(x.value()).isValueSecret(x.isValueSecret()).build());

            HttpParameters httpParameters = HttpParameters.builder()
                    .headerParameters(headerParameters)
                    .bodyParameters(bodyParameters)
                    .queryStringParameters(queryStringParameters)
                    .build();
